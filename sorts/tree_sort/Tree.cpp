...
Tree::Tree(const int n) { root = std::make_shared<Node<int>>(n); }

Tree::Tree(Node<int> *n) { root = std::make_shared<Node<int>>(n->payload); }
...
std::vector<int> Tree::get_contents() const { return contents; }
